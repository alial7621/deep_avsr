...
class AVNet(nn.Module):
    ...
    def __init__(self, dModel, nHeads, numLayers, peMaxLen, inSize, fcHiddenSize, dropout, numClasses):
        ...
        # modified section "3D convolution plus resnet network for extracting visual features"
    ...
    def forward(self, inputBatch):
        ...
        # modified section "Adding video feature extractor from visual frontend"
        if videoInputBatch is not None:
            videoInputBatch = videoInputBatch.transpose(0, 1).transpose(1, 2)
            videoInputBatch = torch.squeeze(videoInputBatch, dim=3)
            batchsize = videoInputBatch.shape[0]
            batch = self.frontend3D(videoInputBatch)
            ...
            outputBatch = torch.squeeze(outputBatch, dim=1)
            videoBatch = self.positionalEncoding(outputBatch)
